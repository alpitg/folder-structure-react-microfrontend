...
const Sidebar = (props: { isOpen: boolean; toggleSidebar: () => void }) => {
  ...
  // build filtered menu with isSelected for each route/subRoute
  const buildMenuState = () => {
    return routes
      .filter((route) => hasPermission(userClaims, route?.claims))
      .map((route) => {
        const subRoutes = route?.subRoutes
          ?.filter((sub) => hasPermission(userClaims, sub?.claims))
          .map((sub) => ({
            ...sub,
            isSelected: location.pathname.startsWith(sub.path),
          }));

        // parent is selected if its path matches or any subRoute selected
        const isSelected =
          location.pathname.startsWith(route.path) ||
          subRoutes?.some((sub) => sub.isSelected);
  ...
  }, [location.pathname, userClaims.join(",")]);
                  ...
                  {route.subRoutes && route.subRoutes.length > 0 && (
                    <div
                      className={`menu-sub menu-sub-accordion ${
                        route.isSelected ? "show" : "collapse"
                      }`}
                    >
                      {route.subRoutes.map((sub) => (
                        <div
                          className={"menu-item " + sub.id}
                          key={sub.id}
                          onClick={(e) => {
                            e.stopPropagation();
                            handleMenuClick(sub);
